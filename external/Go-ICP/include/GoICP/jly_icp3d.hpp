...
// A custom data set class to use nanoflann
    ...
    T ICP3D<T>::Run(T *data, size_t n, Matrix &R, Matrix &t, size_t max_iter, T err_diff)
    {
        ...
        for (iter = 0; iter < max_iter; iter++) {
            ...
            for (i = 0; i < num; i++) {
                // set model point
                p_m.val[i][0] = model_.pts[points[i].id_model].x;
                mu_m.val[0][0] += p_m.val[i][0];
                p_m.val[i][1] = model_.pts[points[i].id_model].y;
                mu_m.val[0][1] += p_m.val[i][1];
                p_m.val[i][2] = model_.pts[points[i].id_model].z;
                mu_m.val[0][2] += p_m.val[i][2];

                idx = points[i].id_data * 3;
                // set query point
                p_d.val[i][0] = r00 * data[idx + 0] + r01 * data[idx + 1] + r02 * data[idx + 2] + t0;
                mu_d.val[0][0] += p_d.val[i][0];
                p_d.val[i][1] = r10 * data[idx + 0] + r11 * data[idx + 1] + r12 * data[idx + 2] + t1;
                mu_d.val[0][1] += p_d.val[i][1];
                p_d.val[i][2] = r20 * data[idx + 0] + r21 * data[idx + 1] + r22 * data[idx + 2] + t2;
                mu_d.val[0][2] += p_d.val[i][2];
