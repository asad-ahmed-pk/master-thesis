...
// Inner Branch-and-Bound, iterating over the translation space
			...
			for (j = 0; j < 8; j++) {
				...
				if (ub < optError) {
					...
					if (error < optError) {
						...
						std::cout << "Error*: " << error << "(ICP " << (double) (clock() - clockBeginICP) / CLOCKS_PER_SEC
							 << "s)" << std::endl;
					}
	...
	void GoICP::ReadConfig(const std::string &filepath)
	{
        // Open and parse the associated config file
        ConfigMap config(filepath.c_str());

        this->MSEThresh = config.getF("MSEThresh");
        this->initNodeRot.a = config.getF("rotMinX");
        this->initNodeRot.b = config.getF("rotMinY");
        this->initNodeRot.c = config.getF("rotMinZ");
        this->initNodeRot.w = config.getF("rotWidth");
        this->initNodeTrans.x = config.getF("transMinX");
        this->initNodeTrans.y = config.getF("transMinY");
        this->initNodeTrans.z = config.getF("transMinZ");
        this->initNodeTrans.w = config.getF("transWidth");
        this->trimFraction = config.getF("trimFraction");
        // If < 0.1% trimming specified, do no trimming
        if(this->trimFraction < 0.001)
        {
            this->doTrim = false;
        }
        this->dt.SIZE = config.getI("distTransSize");
        this->dt.expandFactor = config.getF("distTransExpandFactor");
	}
}